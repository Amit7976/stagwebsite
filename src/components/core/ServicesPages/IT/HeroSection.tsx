...
const Hero: React.FC<HeroProps> = ({
  heading = (
    <>
      Websites & Web <span className="text-orange-600">Development</span>
    </>
  ),
  subheading = "Increase impact, not effort.",
  stacksList = defaultStacks,
}) => {
  const [index, setIndex] = React.useState(0);

  React.useEffect(() => {
    const interval = setInterval(() => {
      setIndex((prev) => (prev + 1) % stacksList.length);
      ...
      <div className="container relative z-10 flex size-full max-w-[103.125rem] flex-col justify-between pb-14 pt-24 md:justify-end">
        <div className="flex h-full flex-col justify-between gap-6 md:justify-end px-10 pb-0">
          ...
          <div className="flex flex-col gap-8">
            <p className="text-foreground text-lg lg:text-2xl">{subheading}</p>
