        ...
        <div className="flex items-center gap-x-5 gap-y-3 flex-wrap my-10">
          {loadingCategories
            ...
            : blogsCategories.map((category, index) => (
                <Link
                  href={`/s/blogs/${category.category}`}
                  key={index}
                  className="text-base font-medium p-3 px-10 bg-gray-100 dark:bg-neutral-800 rounded-full"
                >
                  {category.category}{" "}
                  <span className="opacity-60 text-sm pl-1">
                    ({category.count})
                  </span>
                </Link>
              ))}
